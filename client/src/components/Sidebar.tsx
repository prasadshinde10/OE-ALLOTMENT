...
export default function Sidebar({ isOpen = false, onClose }: SidebarProps) {
  ...
  const adminLinks = [
    {
      href: '/admin/elective-dashboard',
      label: 'Elective Dashboard',
      icon: (
  ...
  return (
    <>
      ...
      <aside className="w-[240px] bg-teal-800 text-white flex-shrink-0 hidden md:flex flex-col h-[calc(100vh-57px)] sticky top-[57px] border-r border-teal-900 shadow-sm">
        {navContent}
      </aside>
      ...
      {/* Mobile drawer */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-[240px] bg-teal-800 text-white transform transition-transform duration-300 ease-in-out md:hidden shadow-xl ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex items-center justify-between px-4 py-4 border-b border-teal-700">
          <span className="text-base font-bold text-white tracking-wide">MIT CSN</span>
          <button
            onClick={onClose}
            className="p-1 text-teal-200 hover:text-white rounded focus:outline-none"
